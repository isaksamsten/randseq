...
/**
 * *
 * This is an implementation of the GoKrimp and SedKrimp algorithms.
...
public class AlgoGoKrimp {
    ...
    static final int NRELATED=1000; // the maximum number of candidate events as starting points for extending to find compressing patterns
    
    BufferedWriter writer; // object to write output file. If null, result is printed to console. Otherwise, the result is written to a file.
    ...
    public void gokrimp() throws IOException{
        ...
        initialization();
        ArrayList<MyPattern> ie=get_Initial_Patterns(); // get a set of initial events
    ...
    void addPattern(MyPattern pattern){
        ...
        HashMap<Integer,Integer> hm = new HashMap<Integer,Integer>(); // hm contains event id and the number of time the event occurs in the pattern.ids
    ...
    */
    ...
    int getBestPattern(){
       ...
       for(int i=0;i<candidates.size();i++){//for every candidate
           ...
           if(candidates.get(i).freq==0) //skip the candidate that does not occurr in the data
               continue;
    ...
    /**
     * get related events of the event @param e, sign-test is used to select these events
     ...
     * @return the number of bits in the binary representation of the input integer a using the Elias code
     */
    int bits(Integer a){
        if(a.intValue()<0)
            return 0;
